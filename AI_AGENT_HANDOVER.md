...
3. **UI 表示および学習ロジック (`PracticeScreen.kt`, `PracticeViewModel.kt`)**:
   ...
   - **街頭インタビュー動画カード (`spokenCorpusExample`)**:
     - ネイティブのリアルな生セリフ（`speakerLine`）と公式 YouTube 起動ボタン（`🎬 YouTube で動画を視聴する ↗`）を配置。
     - YouTube 利用規約（ToS / 埋め込みポリシー）および安定した高画質再生に 100% 準拠するため、アプリ内独自埋め込みではなく公式 YouTube アプリ（または Chrome ブラウザ）への正規 Intent 遷移（`Intent.ACTION_VIEW`）を採用。
