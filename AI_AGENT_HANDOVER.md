...
3. **UI 表示および学習ロジック (`PracticeScreen.kt`, `PracticeViewModel.kt`)**:
   ...
   - **街頭インタビュー動画カード (`spokenCorpusExample`)**:
     ...
     - **右上にコンパクト配置されたアクションアイコン**:
       - **コピーアイコン (`ContentCopy`)**: 生セリフ（`speakerLine`）をクリップボードにコピー（Toast 表示）。
   ...
   - **本文カードおよびアクションバーの整理**:
     - 英文カード右上に **コピーアイコン (`ContentCopy`)**、**翻訳アイコン (`Translate`)**、**表示切替アイコン (`Visibility`)** を集約。
     - ボトムバー（TTS 再生ボタンの横）の重複していた翻訳ボタンは完全削除。
